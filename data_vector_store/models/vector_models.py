...
class BioDataEmbedding(BaseModel):
    ...
    @classmethod
    def validate_dimension_consistency(cls, v, info):
        """Ensure vector dimension matches actual vector length."""
        if info.data.get("embedding_vector") and len(info.data["embedding_vector"]) != v:
...
class SearchQuery(BaseModel):
    ...
    @classmethod
    def validate_query_embedding(cls, v, info):
        """Ensure at least one query input is provided."""
        if v is None and not info.data.get("query_text") and not info.data.get("query_bio_data_id"):
            raise ValueError("At least one of query_bio_data_id, query_embedding, or query_text must be provided")
        return v

    @field_validator("query_text")
    @classmethod
    def validate_query_text(cls, v, info):
        """Ensure at least one query input is provided."""
        if v is None and not info.data.get("query_embedding") and not info.data.get("query_bio_data_id"):
            raise ValueError("At least one of query_bio_data_id, query_embedding, or query_text must be provided")
        return v

    @field_validator("query_bio_data_id")
    @classmethod
    def validate_query_bio_data_id(cls, v, info):
        """Ensure at least one query input is provided."""
        if v is None and not info.data.get("query_embedding") and not info.data.get("query_text"):
            raise ValueError("At least one of query_bio_data_id, query_embedding, or query_text must be provided")
        return v
...
class IndexMetadata(BaseModel):
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        my_count = sum(1 for bd in self.id_to_metadata.values() if bd.bio_data_type == "my_biodata")
        ppl_count = sum(1 for bd in self.id_to_metadata.values() if bd.bio_data_type == "ppl_biodata")

        return {"total": self.total_vectors, "my_biodata": my_count, "ppl_biodata": ppl_count}
